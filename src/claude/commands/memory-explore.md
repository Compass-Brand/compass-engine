...
- Limit exploration to maxNodes=100 nodes and a reasonable time budget (~60 seconds)
- When maxNodes=100 is reached: Stop expanding the graph and proceed directly to summarization with nodes collected so far
- When ~60 second budget is exceeded: Finish the current phase, then skip to summarization
- In both cases, include a note in the summary: "Exploration limited: [X] nodes in [Y] seconds. Results may be incomplete."
