...
1. `../framework/current/phase-state.yaml` is current and reflects the real phase state.
2. `../framework/current/phase.md` is current and aligned with `phase-state.yaml`.
...
7. In workspace or orchestration scope, `../framework/current/initiative-index.yaml` accurately reflects which initiative workstreams are complete, blocked, or still active.

## Closeout Write Order

1. Confirm final phase slug and completion date.
2. Create target folders:
   - `../framework/previous/<phase-slug>-<YYYY-MM-DD>/`
   - `../framework/lessons/<phase-slug>-<YYYY-MM-DD>/`
3. Update `../framework/current/phase-state.yaml` first:
   - set closeout status
   - record archive targets
   - record final workflow and checkpoint state
4. Update `../framework/current/phase.md` second:
   - finalize closeout notes
   - confirm final scope, deliverables, and decisions
5. Update `../framework/roadmap/roadmap.yaml`:
   - set the closed phase status
   - set `actual_end`
   - set `snapshot_path`
   - set `lessons_path`
   - select the next active phase if one exists
6. Update `../framework/roadmap/roadmap.md` to match approved roadmap state.
7. Move superseded roadmap artifacts into the matching archive lanes:
   - `../framework/roadmap/archive/brainstorming/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/research/market/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/research/domain/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/research/technical/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/strategy/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/storytelling/<YYYY-MM-DD>/`
   - `../framework/roadmap/archive/product-brief/<YYYY-MM-DD>/`
8. Move the frozen `../framework/current/` contents into `../framework/previous/<phase-slug>-<YYYY-MM-DD>/`.
9. Record lessons in `../framework/lessons/<phase-slug>-<YYYY-MM-DD>/`.
...
11. Recreate a clean `../framework/current/` scaffold for the next active slice.
12. In workspace or orchestration scope, archive closed initiative workstreams and keep only still-active initiative folders in `../framework/current/initiatives/`.
...
3. Closed-phase artifacts exist only in `../framework/previous/<phase-slug>-<YYYY-MM-DD>/`.
4. Lesson output exists and is actionable.
5. `../framework/current/` contains only next-phase starter structure.
