...
- **Planning lane split** — Route broad strategic artifacts to `planning/framework/roadmap/...` and active delivery artifacts to `planning/framework/current/...`
- **Documentation lane split** — Route product documentation to `docs/{architecture|development|getting-started|guides|reference}/...` and policy/template governance to `docs/human/{policies|templates}/...`
- **Phase governance** — Recommend phase metadata sync before planning execution and phase closeout at end-of-phase/release gates
...
1. **Load catalog** — Load `{project-root}/reference/BMAD/_config/bmad-help.csv`

2. **Resolve output locations and config** — Scan each active authored module folder under `{project-root}/reference/BMAD/modules/custom/` for `config.yaml`. For each workflow row, resolve its `output-location` variables against that module's config so artifact paths can be searched. Also extract `communication_language` and `project_knowledge` from each scanned module's config.
...
6. **Present recommendations** — Show next steps based on:
   ...
   **Routing recommendation overlay**:
   - If user asks for broad ideation/research, prefer roadmap-lane workflows
   - If user asks for sprint/epic execution support, prefer implementation-lane workflows (`implementation-brainstorming`, `implementation-research`)
   - If user asks for documentation migration/standardization, recommend `/bmad-bmm-init-docs`
   - At planning and solutioning checkpoints, recommend `/bmad-bmm-update-docs`
   - Before phase closeout/release readiness, recommend `/bmad-bmm-validate-docs`
   - At phase kickoff, recommend `phase-sync`; at phase completion, recommend `phase-closeout`
