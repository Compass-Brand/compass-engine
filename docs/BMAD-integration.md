...
- Existing Compass consumers (`compass-forge`, `compass-services`, etc.) are
  unaffected — they continue to receive the full merged bundle via `push.js`
  and pick up the new marketplace file transparently through the new
  `claude-plugin` managed target.

`marketplace.json` is generated by `tools/generate-marketplace.js` during
`tools/build.js` and committed to the repo root. The vendored
`tools/vendor/plugin-resolver.cjs` powers `generateBmadHelp()` and is
CI-gated against accidental network-manager imports via
`test/test-no-network-managers.js`.
